...
export default function SkillsFilterPage() {
  const [selectedSkill, setSelectedSkill] = useLocalStorage<string>("skills-filter-selected", "");

  const allSkills = Array.from(new Set(members.flatMap((m: Member) => m.skills)));
    ...
    <div className="space-y-6">
      ...
      {filteredMembers.length === 0 ? (
        <div className="text-center p-8 border border-dashed rounded-lg bg-muted/10">
          ...
          <p className="text-sm text-muted-foreground mt-1">Nobody has the skill "{selectedSkill}" yet.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredMembers.map((member: Member) => (
            <Card key={member.id} className="p-4 flex flex-col justify-between h-full border rounded-lg shadow-sm">
              <div>
